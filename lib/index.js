...
module.exports = function (options) {
  ...
  function hook_listen_redis (args, done) {
    ...
    var redis_in = Redis.createClient(listen_options)
    var redis_out = Redis.createClient(listen_options)
  ...
  function hook_client_redis (args, clientdone) {
    ...
    function make_send (spec, topic, send_done) {
      // Topic can be something like "pin:something" to match listener pin
      if (args.topic) topic = 'seneca_' + args.topic.replace(':', '_') + '_';

      // if Redis url is provided use it instead for production secure redis server
      var redis_in = Redis.createClient(client_options)
      var redis_out = Redis.createClient(client_options)
